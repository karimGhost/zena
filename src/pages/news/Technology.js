...
export default function Technology() {
  ...
  const fetchUrl = requests.fetchTechnology;
...
<PagesData off={off} setoff={setoff}  da={da} />
    ...
    <h2 className="H23">Technology</h2>
          ...
          {currentNews.map(news => (
            <div className="more_news_tab">
              ...
              <p className="descript">
                ...
                <br />
                {news.description}
                <Link to="/news/Technology"><button className='readmore' onClick={() => locale(news.url)}>...Read More</button>
</Link>
              </p>
            </div>
          ))}
